...
extern "C" {
    ...
    fn show_date(format: *const libc::c_char, when: timespec, tz: timezone_t) -> bool;
}
...
unsafe extern "C" fn batch_convert(
    ...
    loop {
        ...
        if line_length < 0 as libc::c_int as libc::c_long {
            ...
            ok = (ok as libc::c_int & show_date_helper(format, when, tz) as libc::c_int)
                as bool;
        }
    }
...
unsafe fn main_0(
    ...
    if !batch_file.is_null() {
        ...
        ok = (ok as libc::c_int & show_date_helper(format_res, when, tz) as libc::c_int)
            as bool;
    }
...
unsafe extern "C" fn show_date_helper(
    ...
    let mut ok: bool = show_date(format, when, tz);
    if format == rfc_email_format.as_ptr() {
