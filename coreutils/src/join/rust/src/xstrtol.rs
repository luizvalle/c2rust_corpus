...
unsafe extern "C" fn bkm_scale(
    ...
    if if (0 as libc::c_int as libc::c_long) < -(1 as libc::c_int) as libc::c_long
        ...
        && (if scale_factor < 0 as libc::c_int {
            if *x < 0 as libc::c_int as libc::c_long {
                ...
                }) != 0 && scale_factor == -(1 as libc::c_int)
                {
                    ...
                    ((0 as libc::c_int as libc::c_long / scale_factor as libc::c_long)
                        < *x) as libc::c_int
                }
            }
        } else {
            if scale_factor == 0 as libc::c_int {
                ...
                if *x < 0 as libc::c_int as libc::c_long {
                    ...
                    ((-(1 as libc::c_int) as libc::c_long / scale_factor as libc::c_long)
                        < *x) as libc::c_int
                }
            }
        }) != 0
    {
        ...
        *(&mut scaled as *mut libc::c_long) = fresh4;
        1 as libc::c_int
    } else {
        let (fresh6, fresh7) = (*x).overflowing_mul(scale_factor.into());
        *(&mut scaled as *mut libc::c_long) = fresh6;
