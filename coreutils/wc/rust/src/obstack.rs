...
unsafe extern "C" fn _obstack_begin_worker(
    ...
    (*h)
        ...
        .as_mut_ptr()
        .offset(
            ((((*chunk).contents).as_mut_ptr() as uintptr_t).wrapping_neg()
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_begin(
    mut h: *mut obstack,
    mut size: size_t,
    mut alignment: size_t,
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_begin_1(
    mut h: *mut obstack,
    mut size: size_t,
    mut alignment: size_t,
    mut chunkfun: Option::<
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_newchunk(mut h: *mut obstack, mut length: size_t) {
    ...
    object_base = ((*new_chunk).contents)
        .as_mut_ptr()
        .offset(
            ((((*new_chunk).contents).as_mut_ptr() as uintptr_t).wrapping_neg()
                & (*h).alignment_mask) as isize,
        );
    memcpy(
    ...
    if (*h).maybe_empty_object() == 0
        && (*h).object_base
            == ((*old_chunk).contents)
                .as_mut_ptr()
                .offset(
                    ((((*old_chunk).contents).as_mut_ptr() as uintptr_t).wrapping_neg()
                        & (*h).alignment_mask) as isize,
                )
    {
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_allocated_p(
    mut h: *mut obstack,
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_free(
    mut h: *mut obstack,
    mut obj: *mut libc::c_void,
) {
    let mut lp: *mut _obstack_chunk = 0 as *mut _obstack_chunk;
...
#[no_mangle]
pub unsafe extern "C" fn rpl_obstack_memory_used(mut h: *mut obstack) -> size_t {
    let mut lp: *mut _obstack_chunk = 0 as *mut _obstack_chunk;
