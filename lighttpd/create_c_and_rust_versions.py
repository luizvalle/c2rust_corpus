...
def _parse_arguments() -> Dict[str, str]:
    ...
    parser.add_argument("--corpus_dir",
                        type=str,
                        help=("The parent directory that will contain the C"
                              " and Rust versions"),
                        default=os.getcwd())
    parser.add_argument("--lighttpd_dir",
                        type=str,
                        help="The root directory for lighttpd.",
                        required=True)
...
def _get_deps(filepath: str) -> Dependencies:
    ...
    if (not os.path.exists(deps_filepath)
        ...
        alternative_deps_filepath = os.path.join(
            directory, ".deps", f"lighttpd-{filename}.Po")
        if (not os.path.exists(alternative_deps_filepath)
                or not os.path.isfile(alternative_deps_filepath)):
            _LOGGER.error(f"Could not find the dependency file:"
                          " '{deps_filepath}' or '{alternative_deps_filepath}'")
            sys.exit(1)
        deps_filepath = alternative_deps_filepath
...
def _copy_deps_to_dir(
    deps: Dependencies, out_c_dir: str, lighttpd_dir: str) -> None:
    src_filepath = os.path.join(lighttpd_dir, "src", deps.source_file)
    ...
    for header_file in deps.header_files:
        header_filepath = os.path.join(lighttpd_dir, "src", header_file)
...
def _map_symbols_to_dependencies(dir_to_search: str) -> Dict[str, Dependencies]:
    ...
    for object_filepath in glob.glob(
        ...
        filename = os.path.basename(object_filepath)
        if filename.startswith("t_test_") or filename.startswith("test_"):
...
def main():
    ...
    if not os.path.exists(args["lighttpd_dir"]):
        _LOGGER.error(
            f"The lighttpd directory does not exist: {args['lighttpd_dir']}")
        sys.exit(1)

    out_prog_dir = os.path.join(args["corpus_dir"], "lighttpd")
    ...
    _run_command(f"make --directory={args['lighttpd_dir']}")

    _LOGGER.info(f"Compiled the source files in {args['lighttpd_dir']}")

    syms_to_deps = _map_symbols_to_dependencies(
        os.path.join(args["lighttpd_dir"], "src"))
    ...
    main_source_filepath = os.path.join(
        args["lighttpd_dir"], "src", "server.c")
    deps = _get_deps(main_source_filepath)
    _copy_deps_to_dir(deps, out_c_dir, args["lighttpd_dir"])
    ...
    while True:
        make_output, _ = _run_command(
            f"make -i --directory=\"{out_c_dir}\" lighttpd 2>&1")
        ...
        if num_missing_files:
            for file in missing_files:
                filepath, _ = _run_command(
                    f"find {args['lighttpd_dir']} -type f -name \"{file}\""
                    " | head -n 1")
    ...
    mk_output, mk_retcode = _run_command(
        f"make --directory={out_c_dir} lighttpd 2>&1")
    if mk_retcode:
        _LOGGER.error(
            f"Could not build the C version of lighttpd:\n{mk_output}"
        )
        sys.exit(1)

    _LOGGER.info("Successfully built the C version of lighttpd")
    ...
    ib_out, ib_retcode = _run_command(
        ...
        f" make -s --directory={out_c_dir} lighttpd 2>&1"
    )
    if ib_retcode:
    ...
    c2r_out, c2r_retcode = _run_command(
        "c2rust transpile --binary lighttpd --overwrite-existing"
    ...
    if cb_retcode:
        ...
        sys.exit(1)
    _LOGGER.info(
        f"Successfully built the Rust version of lighttpd")
