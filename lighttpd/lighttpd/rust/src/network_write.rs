...
extern "C" {
    ...
    fn chunk_file_pread_chunk(
        c: *mut chunk,
        buf: *mut libc::c_void,
        count: size_t,
    ) -> ssize_t;
    fn chunk_open_file_chunk(c: *mut chunk, errh: *mut log_error_st) -> libc::c_int;
...
unsafe extern "C" fn network_write_error(
    ...
    match *__errno_location() {
        ...
        _ => {
            log_perror(
                errh,
                b"network_write.c\0" as *const u8 as *const libc::c_char,
                114 as libc::c_int as libc::c_uint,
                b"write() %d\0" as *const u8 as *const libc::c_char,
                fd,
            );
...
unsafe extern "C" fn network_write_file_chunk_no_mmap(
    ...
    let mut toSend: off_t = (*c).file.length - (*c).offset;
    ...
    {
        toSend = ::core::mem::size_of::<[libc::c_char; 16384]>() as libc::c_ulong
            as off_t;
    }
...
unsafe extern "C" fn network_writev_mem_chunks(
    ...
    {
        let mut c_len: off_t = buffer_clen((*c).mem) as off_t - (*c).offset;
        if c_len > 0 as libc::c_int as libc::c_long {
            if c_len > *p_max_bytes - toSend {
                c_len = *p_max_bytes - toSend;
            }
            toSend += c_len;
...
unsafe extern "C" fn network_write_file_chunk_sendfile(
    ...
    if nbytes <= 0 as libc::c_int as libc::c_long {
        return network_remove_finished_chunks(cq, nbytes);
    }
    wr = sendfile(fd, (*c).file.fd, &mut offset, nbytes as size_t);
    if wr > 0 as libc::c_int as libc::c_long {
        written = wr;
    }
    ...
    return if wr >= 0 as libc::c_int as libc::c_long && written == nbytes {
        0 as libc::c_int
    } else {
        -(3 as libc::c_int)
    };
