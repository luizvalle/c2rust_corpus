    ...
    while i < used {
        if encoded_chars_http_uri_reqd[*s.offset(i as isize) as usize] == 0 {
            *p.offset(j as isize) = *s.offset(i as isize);
            if (*s.offset(i as isize) as libc::c_int == '?' as i32) as libc::c_int
                as libc::c_long != 0 && -(1 as libc::c_int) == qs
            {
                qs = j;
            }
...
unsafe extern "C" fn burl_normalize_basic_required_fix(
    ...
    let mut n1: libc::c_uint = 0;
    let mut n2: libc::c_uint = 0;
    let mut invalid_utf8: libc::c_int = 0 as libc::c_int;
    memcpy(p as *mut libc::c_void, s as *const libc::c_void, i as size_t);
    while i < used {
        if encoded_chars_http_uri_reqd[*s.offset(i as isize) as usize] == 0 {
            *p.offset(j as isize) = *s.offset(i as isize);
            ...
            }
        {
                ...
                }) != 0
            {
                ...
                *p.offset(j as isize) = hex_chars_uc[n2 as usize] as libc::c_uchar;
                invalid_utf8
                    |= (x >= 0xf5 as libc::c_int as libc::c_uint
                        || x | 0x1 as libc::c_int as libc::c_uint
                            == 0xc1 as libc::c_int as libc::c_uint) as libc::c_int;
            }
            i += 2 as libc::c_int;
        } else {
            if *s.offset(i as isize) as libc::c_int == '#' as i32 {
            ...
            *p
                ...
                & 0xf as libc::c_int) as usize] as libc::c_uchar;
            invalid_utf8
                |= (*s.offset(i as isize) as libc::c_int >= 0xf5 as libc::c_int
                    || *s.offset(i as isize) as libc::c_int | 0x1 as libc::c_int
                        == 0xc1 as libc::c_int) as libc::c_int;
        }
        i += 1;
        i;
        j += 1;
    ...
    buffer_copy_string_len(b, p as *mut libc::c_char, j as size_t);
    return if invalid_utf8 == 0 { qs } else { -(2 as libc::c_int) };
}
...
#[cold]
#[inline(never)]
unsafe extern "C" fn burl_scan_qmark(b: *const buffer) -> libc::c_int {
    let qmark: *const libc::c_char = strchr((*b).ptr, '?' as i32);
    return if !qmark.is_null() {
        qmark.offset_from((*b).ptr) as libc::c_long as libc::c_int
    } else {
        -(1 as libc::c_int)
    };
}
...
pub unsafe extern "C" fn burl_normalize(
    ...
    if -(2 as libc::c_int) == qs {
        if flags & HTTP_PARSEOPT_URL_NORMALIZE_INVALID_UTF8_REJECT as libc::c_int != 0 {
            return -(2 as libc::c_int);
        }
