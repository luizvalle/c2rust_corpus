...
static mut http_range_allow_http10: libc::c_int = 0;
#[no_mangle]
#[cold]
pub unsafe extern "C" fn http_range_config_allow_http10(mut flag: libc::c_int) {
    http_range_allow_http10 = flag;
}
#[cold]
#[inline(never)]
unsafe extern "C" fn http_range_coalesce_unsorted(
    ranges: *mut off_t,
    mut n: libc::c_int,
) -> libc::c_int {
...
unsafe extern "C" fn http_range_parse(
    ...
    let mut lim: libc::c_int = 128 as libc::c_int * 2 as libc::c_int;
    loop {
                ...
                != -(1 as libc::c_int) as libc::c_long
        {
            ...
            if !(n < 4 as libc::c_int) {
                if *ranges.offset((n - 4 as libc::c_int) as isize)
                    <= *ranges.offset((n - 2 as libc::c_int) as isize)
                {
                    if !(*ranges.offset((n - 3 as libc::c_int) as isize)
                        < *ranges.offset((n - 2 as libc::c_int) as isize)
                            - 80 as libc::c_int as libc::c_long)
                    {
                            ...
                            < *ranges.offset((n - 1 as libc::c_int) as isize)
                        {
                            *ranges
                                .offset(
                                    (n - 3 as libc::c_int) as isize,
                                ) = *ranges.offset((n - 1 as libc::c_int) as isize);
                        }
                        n -= 2 as libc::c_int;
                    }
                } else if n > 10 as libc::c_int * 2 as libc::c_int {
                    n -= 2 as libc::c_int;
                    break;
                } else {
                    lim = 10 as libc::c_int * 2 as libc::c_int;
                }
            }
        } else if 1 as libc::c_int as libc::c_long != 0 {
        ...
        let fresh0 = s;
        s = s.offset(1);
        if !(*fresh0 as libc::c_int != '\0' as i32 && n < lim) {
            break;
        }
    }
    ...
    return if lim == 128 as libc::c_int * 2 as libc::c_int {
        n
    } else {
        http_range_coalesce_unsorted(ranges, n)
    };
}
unsafe extern "C" fn http_range_single(r: *mut request_st, mut ranges: *const off_t) {
    ...
    let mut len: uint32_t = (::core::mem::size_of::<[libc::c_char; 7]>()
        as libc::c_ulong)
...
unsafe extern "C" fn http_range_multi(
    ...
    while i < n {
        ...
        buffer_append_char(tb, '-' as i32 as libc::c_char);
...
unsafe extern "C" fn http_range_not_satisfiable(
    ...
    let mut len: uint32_t = (::core::mem::size_of::<[libc::c_char; 9]>()
        as libc::c_ulong)
        .wrapping_sub(1 as libc::c_int as libc::c_ulong) as uint32_t;
    let mut cr: [libc::c_char; 32] = *::core::mem::transmute::<
        &[u8; 32],
        &mut [libc::c_char; 32],
...
unsafe extern "C" fn http_range_process(
    ...
    let mut ranges: [off_t; 256] = [0; 256];
