...
static mut network_mptcp: libc::c_int = 0 as libc::c_int;
#[no_mangle]
...
unsafe extern "C" fn network_server_handle_fdevent(
    ...
    loop {
        ...
        } else if addrlen <= 2 as libc::c_int as libc::c_ulong {
            memcpy(
                (addr.un.sun_path).as_mut_ptr() as *mut libc::c_void,
                ((*srv_socket).addr.un.sun_path).as_ptr() as *const libc::c_void,
                if ((*srv_socket).srv_token_colon as libc::c_ulong)
                    < ::core::mem::size_of::<[libc::c_char; 108]>() as libc::c_ulong
                {
                    ((*srv_socket).srv_token_colon as size_t)
...
#[cold]
unsafe extern "C" fn network_abstract_socket_enc(
    abstract_0: *mut buffer,
    sun_path: *mut libc::c_uchar,
    len: uint32_t,
) {
    buffer_clear(abstract_0);
    let mut n: uint32_t = 0 as libc::c_int as uint32_t;
    let mut i: uint32_t = 0 as libc::c_int as uint32_t;
    while i < len {
        n = (n as libc::c_uint)
            .wrapping_add(
                (if (*sun_path.offset(i as isize) as libc::c_int - 20 as libc::c_int)
                    < 107 as libc::c_int
                {
                    1 as libc::c_int
                } else {
    ...
    let mut s: *mut libc::c_char = buffer_extend(abstract_0, n as size_t);
    let mut i_0: uint32_t = 0 as libc::c_int as uint32_t;
    while i_0 < len {
        if (*sun_path.offset(i_0 as isize) as libc::c_int - 20 as libc::c_int)
            < 107 as libc::c_int
            && *sun_path.offset(i_0 as isize) as libc::c_int != '\\' as i32
        {
            let fresh0 = s;
            s = s.offset(1);
            *fresh0 = *(sun_path as *mut libc::c_char).offset(i_0 as isize);
        } else {
            *s.offset(0 as libc::c_int as isize) = '\\' as i32 as libc::c_char;
            *s.offset(1 as libc::c_int as isize) = 'x' as i32 as libc::c_char;
            *s
                .offset(
            ...
            ))[(*sun_path.offset(i_0 as isize) as libc::c_int >> 4 as libc::c_int)
                as usize];
            *s
                .offset(
            ...
            >(
                b"0123456789abcdef\0",
            ))[(*sun_path.offset(i_0 as isize) as libc::c_int & 0xf as libc::c_int)
                as usize];
...
#[cold]
unsafe extern "C" fn network_abstract_socket_dec(
    abstract_0: *const buffer,
    sun_path: *mut libc::c_char,
    plen: uint32_t,
) -> uint32_t {
    let mut s: *const libc::c_char = (*abstract_0).ptr;
    let mut n: uint32_t = 0 as libc::c_int as uint32_t;
    let mut hi: libc::c_int = 0;
    let mut lo: libc::c_int = 0;
    while *s as libc::c_int != 0 && n < plen {
        if *s.offset(0 as libc::c_int as isize) as libc::c_int != '\\' as i32 {
            let fresh1 = s;
            s = s.offset(1);
            *sun_path.offset(n as isize) = *fresh1;
        } else {
            if !(*s.offset(1 as libc::c_int as isize) as libc::c_int == 'x' as i32
                && {
                    hi = hex2int(*s.offset(2 as libc::c_int as isize) as libc::c_uchar)
                        as libc::c_int;
                    hi != 0xff as libc::c_int
                }
                && {
                    lo = hex2int(*s.offset(3 as libc::c_int as isize) as libc::c_uchar)
                        as libc::c_int;
                    lo != 0xff as libc::c_int
                })
            {
                break;
            }
            *sun_path.offset(n as isize) = (hi << 4 as libc::c_int | lo) as libc::c_char;
            s = s.offset(4 as libc::c_int as isize);
        }
        n = n.wrapping_add(1);
        n;
...
#[cold]
unsafe extern "C" fn network_abstract_socket_parse(
    mut abstract_0: *const buffer,
    ...
    (*addr).un.sun_family = 1 as libc::c_int as sa_family_t;
    let mut len: uint32_t = (*addr_len as libc::c_ulong).wrapping_sub(2 as libc::c_ulong)
        as uint32_t;
    if len as libc::c_ulong
        > ::core::mem::size_of::<[libc::c_char; 108]>() as libc::c_ulong
    {
        len = ::core::mem::size_of::<[libc::c_char; 108]>() as libc::c_ulong as uint32_t;
    }
    len = network_abstract_socket_dec(
        abstract_0,
        ((*addr).un.sun_path).as_mut_ptr(),
        len,
    );
    if len != 0 {
        *addr_len = (2 as libc::c_ulong).wrapping_add(len as libc::c_ulong) as socklen_t;
        ...
        log_error(
            ...
            b"abstract unix socket filename invalid encoding or too long: %s\0"
...
unsafe extern "C" fn network_host_normalize_addr_str(
    mut host: *mut buffer,
    mut addr: *mut sock_addr,
    mut addr_len: socklen_t,
) {
    buffer_clear(host);
    sock_addr_stringify_append_buffer(host, addr);
    if 1 as libc::c_int == sock_addr_get_family(addr)
        && 0 as libc::c_int as libc::c_uint == buffer_clen(host)
    {
        network_abstract_socket_enc(
            host,
            ((*addr).un.sun_path).as_mut_ptr() as *mut libc::c_uchar,
            (addr_len as libc::c_ulong).wrapping_sub(2 as libc::c_ulong) as uint32_t,
        );
    }
}
unsafe extern "C" fn network_host_parse_addr(
    ...
    if *h.offset(0 as libc::c_int as isize) as libc::c_int == '/' as i32
        || *h.offset(0 as libc::c_int as isize) as libc::c_int == '\\' as i32
    {
        if *h.offset(0 as libc::c_int as isize) as libc::c_int == '\\' as i32
            && *h.offset(1 as libc::c_int as isize) as libc::c_int == 'x' as i32
            && *h.offset(2 as libc::c_int as isize) as libc::c_int == '0' as i32
            && *h.offset(3 as libc::c_int as isize) as libc::c_int == '0' as i32
        {
...
unsafe extern "C" fn network_srv_sockets_append(
    ...
    let srv_sockets: *mut server_socket_array = &mut (*srv).srv_sockets;
    if (*srv_sockets).used & (4 as libc::c_int - 1 as libc::c_int) as libc::c_uint == 0 {
        ck_realloc_u32(
            &mut (*srv_sockets).ptr as *mut *mut *mut server_socket
                as *mut *mut libc::c_void,
            (*srv_sockets).used as size_t,
            4 as libc::c_int as size_t,
            ::core::mem::size_of::<*mut server_socket>() as libc::c_ulong,
        );
    }
    let fresh4 = (*srv_sockets).used;
    (*srv_sockets).used = ((*srv_sockets).used).wrapping_add(1);
    let ref mut fresh5 = *((*srv_sockets).ptr).offset(fresh4 as isize);
    *fresh5 = srv_socket;
}
...
unsafe extern "C" fn network_srv_socket_init_token(
    srv_socket: *mut server_socket,
    token: *const buffer,
) {
    (*srv_socket).srv_token = buffer_init();
    let srv_token: *mut buffer = (*srv_socket).srv_token;
    buffer_copy_buffer(srv_token, token);
    if 1 as libc::c_int == sock_addr_get_family(&mut (*srv_socket).addr) {
        (*srv_socket).srv_token_colon = buffer_clen(srv_token) as uint8_t;
    } else {
...
unsafe extern "C" fn network_server_init(
    mut srv: *mut server,
    mut s: *const network_socket_config,
    ...
    let mut addr_len: socklen_t = ::core::mem::size_of::<sock_addr>() as libc::c_ulong
        as socklen_t;
    let mut addr: sock_addr = sock_addr {
        ipv6: sockaddr_in6 {
            sin6_family: 0,
    ...
    let mut use_ipv6: libc::c_int = (*s).use_ipv6 as libc::c_int;
    ...
    while i < (*srv).srv_sockets.used {
        ...
        ) != 0
        {
            if !(0 as libc::c_uint) as libc::c_ushort as libc::c_int
                == (**((*srv).srv_sockets.ptr).offset(i as isize)).sidx as libc::c_int
            {
                (**((*srv).srv_sockets.ptr).offset(i as isize))
                    .sidx = sidx as libc::c_ushort;
                (**((*srv).srv_sockets.ptr).offset(i as isize))
                    .is_ssl = (*s).ssl_enabled;
            }
            return 0 as libc::c_int;
        }
        i = i.wrapping_add(1);
    ...
    if -(1 as libc::c_int) != stdin_fd {
        ...
        != network_host_parse_addr(srv, &mut addr, &mut addr_len, host_token, use_ipv6)
    {
        return -(1 as libc::c_int)
    }
    ...
    network_host_normalize_addr_str(host_token, &mut addr, addr_len);
    host = (*host_token).ptr;
    ...
    while i_0 < (*srv).srv_sockets.used {
        ...
        {
            if !(0 as libc::c_uint) as libc::c_ushort as libc::c_int
                == (**((*srv).srv_sockets.ptr).offset(i_0 as isize)).sidx as libc::c_int
            {
                (**((*srv).srv_sockets.ptr).offset(i_0 as isize))
                    .sidx = sidx as libc::c_ushort;
                (**((*srv).srv_sockets.ptr).offset(i_0 as isize))
                    .is_ssl = (*s).ssl_enabled;
            }
        ...
        match *__errno_location() {
            ...
            _ => {
                log_perror(
                    ...
                    b"testing socket failed: connect() %s\0" as *const u8
                        as *const libc::c_char,
                    host,
                );
                return -(1 as libc::c_int);
        ...
        if network_mptcp != 0 {
            ...
            if -(1 as libc::c_int) == (*srv_socket).fd {
                log_pri(
                    ...
                    b"socket() IPPROTO_MPTCP\0" as *const u8 as *const libc::c_char,
                );
                network_mptcp = 0 as libc::c_int;
            }
        }
        if !(-(1 as libc::c_int) != (*srv_socket).fd) {
            (*srv_socket)
            ...
            if -(1 as libc::c_int) == (*srv_socket).fd {
                    ...
                    && *__errno_location() == 97 as libc::c_int
                {
                    log_perror(
                        ...
                        b"ignoring $SERVER[\"socket\"] == \"%s\"; socket(AF_INET6)\0"
                            as *const u8 as *const libc::c_char,
                        host,
                    );
                    return 0 as libc::c_int;
        ...
        if (*s).defer_accept != 0 {
                ...
                )
            {
                log_perror(
                    ...
                    b"setsockopt(TCP_DEFER_ACCEPT)\0" as *const u8 as *const libc::c_char,
                );
            }
        }
    }
...
pub unsafe extern "C" fn network_close(mut srv: *mut server) -> libc::c_int {
    ...
    while i < (*srv).srv_sockets.used {
        ...
        if (*srv_socket).fd != -(1 as libc::c_int) {
            ...
            fdevent_socket_close((*srv_socket).fd);
        }
    ...
    while i_0 < (*srv).srv_sockets_inherited.used {
        ...
        {
            fdevent_socket_close((*srv_socket_0).fd);
        }
...
unsafe extern "C" fn network_socket_activation_nfds(
    ...
    while fd < nfds {
        ...
        if -(1 as libc::c_int) == rc {
            ...
            network_host_normalize_addr_str(host, &mut addr, addr_len);
...
pub unsafe extern "C" fn network_init(
    ...
    network_mptcp = config_feature_bool(
        srv,
        b"server.network-mptcp\0" as *const u8 as *const libc::c_char,
        0 as libc::c_int,
    );
    ...
    {
        (*srv).srvconf.systemd_socket_activation = 1 as libc::c_int as libc::c_uchar;
    }
    let mut rc: libc::c_int = 0 as libc::c_int;
    ...
    if (*srv).srvconf.systemd_socket_activation != 0 {
        ...
        if 0 as libc::c_int != rc {
            current_block_58 = 7419121793134201633;
        } else {
        ...
        11307063007268554308 => {
            if -(1 as libc::c_int) != stdin_fd {
                ...
                rc = if 0 as libc::c_int as libc::c_uint == (*srv).srv_sockets.used {
                    ...
                    fdevent_socket_close(stdin_fd)
                };
                buffer_free(b);
                if 0 as libc::c_int != rc {
                    current_block_58 = 7419121793134201633;
                } else {
                    current_block_58 = 15345278821338558188;
                }
            } else {
                current_block_58 = 15345278821338558188;
            }
                ...
                _ => {
                    ...
                    while i_0 < (*(*srv).config_context).used {
                            ...
                            != cfginfo.comp as libc::c_uint)
                        {
                            ...
                            let ref mut fresh6 = *(&mut host_token as *mut *mut buffer
                                as *mut *const buffer);
                            *fresh6 = cfginfo.string;
                            memcpy(
                                &mut (*p).conf as *mut network_socket_config
                                    as *mut libc::c_void,
                                &mut (*p).defaults as *mut network_socket_config
                                    as *const libc::c_void,
                                ::core::mem::size_of::<network_socket_config>()
                                    as libc::c_ulong,
                            );
                            ...
                            {
                                let mut addr_len: socklen_t = ::core::mem::size_of::<
                                    sock_addr,
                                >() as libc::c_ulong as socklen_t;
                                let mut addr: sock_addr = sock_addr {
                                    ipv6: sockaddr_in6 {
                                ...
                                network_host_normalize_addr_str(
                                    host_token,
                                    &mut addr,
                                    addr_len,
                                );
                    ...
                    if !(0 as libc::c_int != rc) {
                            ...
                            && -(1 as libc::c_int) == stdin_fd
                        {
                            ...
                            if ((*b_0).ptr).is_null()
                                || *((*b_0).ptr).offset(0 as libc::c_int as isize)
                                    ...
                                    && *((*b_0).ptr).offset(0 as libc::c_int as isize)
                                        as libc::c_int != '\\' as i32
                            {
                                buffer_append_char(b_0, ':' as i32 as libc::c_char);
                            ...
                            if 0 as libc::c_int != rc {
                                ...
                                current_block_58 = 7343950298149844727;
                            }
                        } else {
                            current_block_58 = 7343950298149844727;
                        }
                        match current_block_58 {
                            ...
                            _ => {
                                ...
                                while i_2 < (*srv).srv_sockets.used {
                                    if !(0 as libc::c_uint) as libc::c_ushort as libc::c_int
                                        == (**((*srv).srv_sockets.ptr).offset(i_2 as isize)).sidx
                                            as libc::c_int
                                    {
                                        (**((*srv).srv_sockets.ptr).offset(i_2 as isize))
                                            .sidx = 0 as libc::c_int as libc::c_ushort;
                                        (**((*srv).srv_sockets.ptr).offset(i_2 as isize))
                                            .is_ssl = (*p).defaults.ssl_enabled;
                                    }
