...
extern "C" {
    ...
    fn splaytree_splay_nonnull(t: *mut splay_tree, key: libc::c_int) -> *mut splay_tree;
    fn splaytree_insert_splayed(
        t: *mut splay_tree,
        key: libc::c_int,
    ...
    fn splaytree_delete_splayed_node(t: *mut splay_tree) -> *mut splay_tree;
...
#[inline]
unsafe extern "C" fn splaytree_splay(
    mut t: *mut splay_tree,
    mut key: libc::c_int,
) -> *mut splay_tree {
    if t.is_null() || key == (*t).key {
        return t;
    }
...
unsafe extern "C" fn splaytree_djbhash(
    ...
    return djbhash(str, len, 5381 as libc::c_int as uint32_t) as int32_t;
}
#[inline]
...
#[inline(never)]
unsafe extern "C" fn stat_cache_sptree_ndx(
...
unsafe extern "C" fn stat_cache_sptree_find(
    ...
    return stat_cache_sptree_ndx(sptree, 0 as *mut libc::c_int, name, len);
}
unsafe extern "C" fn fam_dir_entry_init(
    mut name: *const libc::c_char,
    mut len: size_t,
) -> *mut fam_dir_entry {
    let fam_dir: *mut fam_dir_entry = ck_calloc(
...
unsafe extern "C" fn fam_dir_periodic_cleanup() {
    ...
    while !((*scf).dirs).is_null() {
        ...
        i = 0 as libc::c_int;
        while i < max_ndx {
            (*scf).dirs = splaytree_splay_nonnull((*scf).dirs, keys[i as usize]);
            let mut fam_dir: *mut fam_dir_entry = (*(*scf).dirs).data
                as *mut fam_dir_entry;
    ...
    loop {
        ...
        if rd <= 0 as libc::c_int {
                ...
                && *__errno_location() != 11 as libc::c_int
            {
                log_perror(
                    (*scf).errh,
                    b"stat_cache.c\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn stat_cache_init_fam(
    ...
    let mut scf: *mut stat_cache_fam = ck_calloc(
        1 as libc::c_int as size_t,
...
unsafe extern "C" fn stat_cache_free_fam(mut scf: *mut stat_cache_fam) {
    ...
    while !((*scf).dirs).is_null() {
        fam_dir_entry_free((*(*scf).dirs).data as *mut fam_dir_entry);
        (*scf).dirs = splaytree_delete_splayed_node((*scf).dirs);
    }
...
unsafe extern "C" fn fam_dir_monitor(
    ...
    let mut dir_ndx: libc::c_int = 0;
    let mut fam_dir: *mut fam_dir_entry = stat_cache_sptree_ndx(
        &mut (*scf).dirs,
        &mut dir_ndx,
        fn_0,
        dirlen,
...
unsafe extern "C" fn stat_cache_entry_init() -> *mut stat_cache_entry {
    let mut sce: *mut stat_cache_entry = ck_calloc(
        1 as libc::c_int as size_t,
...
unsafe extern "C" fn stat_cache_sptree_node_free(
    mut sptree: *mut splay_tree,
) -> *mut splay_tree {
...
pub unsafe extern "C" fn stat_cache_choose_engine(
        ...
        ) != 0
    {
        ...
        static mut fmt: [libc::c_char; 90] = unsafe {
            *::core::mem::transmute::<
                &[u8; 90],
                &[libc::c_char; 90],
            >(
...
pub unsafe extern "C" fn stat_cache_update_entry(
    ...
    if (0 as libc::c_int as libc::c_uint == len) as libc::c_int as libc::c_long != 0 {
        return;
    }
...
unsafe extern "C" fn stat_cache_prune_dir_tree(
    ...
    while !sptree.is_null() {
        ...
        i = 0 as libc::c_int;
        while i < max_ndx {
            sptree = splaytree_splay_nonnull(sptree, keys[i as usize]);
            sptree = stat_cache_sptree_node_free(sptree);
            i += 1;
            i;
        }
...
#[cold]
#[inline(never)]
unsafe extern "C" fn stat_cache_refresh_entry(
    name: *const buffer,
    mut len: uint32_t,
    mut sce: *mut stat_cache_entry,
    file_ndx: libc::c_int,
    ...
    if sce.is_null() || stat_cache_stat_eq(&mut (*sce).st, &mut st) == 0 {
        ...
        if sce.is_null() {
            sce = stat_cache_entry_init();
            buffer_copy_string_len(&mut (*sce).name, (*name).ptr, len as size_t);
            let sptree: *mut splay_tree = sc.files;
            if !sptree.is_null() && (*sptree).key == file_ndx {
                if refresh < 0 as libc::c_int {
                    stat_cache_entry_free((*sptree).data);
                }
                (*sptree).data = sce as *mut libc::c_void;
            } else {
                sc
                    .files = splaytree_insert_splayed(
                    sptree,
                    file_ndx,
                    sce as *mut libc::c_void,
                );
            }
        } else {
            buffer_clear(&mut (*sce).etag);
        ...
        if sc.stat_cache_engine == STAT_CACHE_ENGINE_FAM as libc::c_int {
            ...
            (*sce)
                .fam_dir = fam_dir_monitor(sc.scf, (*name).ptr, len, &mut st)
                as *mut libc::c_void;
        }
    }
    (*sce).stat_ts = log_monotonic_secs;
...
pub unsafe extern "C" fn stat_cache_get_entry(
    ...
    let mut len: uint32_t = buffer_clen(name);
    let mut final_slash: libc::c_int = 0 as libc::c_int;
    if (0 as libc::c_int as libc::c_uint == len) as libc::c_int as libc::c_long != 0 {
        return 0 as *mut stat_cache_entry;
    }
    if *((*name).ptr).offset(len.wrapping_sub(1 as libc::c_int as libc::c_uint) as isize)
        as libc::c_int == '/' as i32
    {
    ...
    let mut file_ndx: libc::c_int = 0;
    let mut sce: *mut stat_cache_entry = stat_cache_sptree_ndx(
        &mut sc.files,
        &mut file_ndx,
        (*name).ptr,
        len,
    ) as *mut stat_cache_entry;
    let mut refresh: libc::c_int = -(1 as libc::c_int);
    if !sce.is_null() {
        if buffer_eq_slen(&mut (*sce).name, (*name).ptr, len as size_t) != 0 {
            let cur_ts: unix_time64_t = log_monotonic_secs;
            refresh = 1 as libc::c_int;
            if sc.stat_cache_engine == STAT_CACHE_ENGINE_SIMPLE as libc::c_int {
                refresh = ((*sce).stat_ts != cur_ts) as libc::c_int;
            } else if sc.stat_cache_engine == STAT_CACHE_ENGINE_FAM as libc::c_int
                && !((*sce).fam_dir).is_null()
            {
                refresh = !(cur_ts - (*sce).stat_ts < 16 as libc::c_int as libc::c_long)
                    as libc::c_int;
            }
        } else {
            sce = 0 as *mut stat_cache_entry;
...
pub unsafe extern "C" fn stat_cache_path_contains_symlink(
    ...
    if (1 as libc::c_int as libc::c_ulong == len) as libc::c_int as libc::c_long != 0 {
        return 0 as libc::c_int;
    }
