...
pub unsafe extern "C" fn array_data_string_init() -> *mut data_string {
    ...
    let mut ds: *mut data_string = ck_calloc(
...
pub unsafe extern "C" fn array_data_integer_init() -> *mut data_integer {
    ...
    let mut di: *mut data_integer = ck_calloc(
        1 as libc::c_int as size_t,
...
pub unsafe extern "C" fn array_data_array_init() -> *mut data_array {
    ...
    let mut da: *mut data_array = ck_calloc(
        1 as libc::c_int as size_t,
...
unsafe extern "C" fn array_extend(a: *mut array, mut n: uint32_t) {
    ...
    (*a)
        .data = ck_realloc_u32(
        &mut (*a).data as *mut *mut *mut data_unset as *mut *mut libc::c_void,
        (*a).size as size_t,
        0 as libc::c_int as size_t,
        ::core::mem::size_of::<*mut data_unset>() as libc::c_ulong,
    ) as *mut *mut data_unset;
    (*a)
        .sorted = ck_realloc_u32(
        &mut (*a).sorted as *mut *mut *mut data_unset as *mut *mut libc::c_void,
        (*a).size as size_t,
        0 as libc::c_int as size_t,
        ::core::mem::size_of::<*mut data_unset>() as libc::c_ulong,
    ) as *mut *mut data_unset;
...
pub unsafe extern "C" fn array_replace(a: *mut array, entry: *mut data_unset) {
    let oldp: *mut *mut data_unset = array_find_or_insert(a, entry);
    if oldp.is_null() {
        return;
    }
    let old: *mut data_unset = *oldp;
    *oldp = entry;
    let mut i: uint32_t = 0 as libc::c_int as uint32_t;
    while i < (*a).used {
        if *((*a).data).offset(i as isize) == old {
            let ref mut fresh5 = *((*a).data).offset(i as isize);
            *fresh5 = entry;
            break;
        } else {
            i = i.wrapping_add(1);
