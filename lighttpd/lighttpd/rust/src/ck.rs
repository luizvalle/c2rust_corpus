...
#[no_mangle]
pub unsafe extern "C" fn ck_malloc(mut nbytes: size_t) -> *mut libc::c_void {
    let mut ptr: *mut libc::c_void = malloc(nbytes);
    if ptr.is_null() {
        ck_assert_failed(
            b"ck.c\0" as *const u8 as *const libc::c_char,
            328 as libc::c_int as libc::c_uint,
            b"NULL != ptr\0" as *const u8 as *const libc::c_char,
        );
    }
    return ptr;
}
#[no_mangle]
pub unsafe extern "C" fn ck_calloc(
    mut nmemb: size_t,
    mut elt_sz: size_t,
) -> *mut libc::c_void {
    let mut ptr: *mut libc::c_void = calloc(nmemb, elt_sz);
    if ptr.is_null() {
        ck_assert_failed(
            b"ck.c\0" as *const u8 as *const libc::c_char,
...
#[no_mangle]
pub unsafe extern "C" fn ck_realloc_u32(
    mut list: *mut *mut libc::c_void,
    mut n: size_t,
    mut x: size_t,
    mut elt_sz: size_t,
) -> *mut libc::c_void {
    if !(x <= 4294967295 as libc::c_uint as libc::c_ulong
        && n <= (4294967295 as libc::c_uint as libc::c_ulong).wrapping_sub(x)
        && n.wrapping_add(x)
            <= (18446744073709551615 as libc::c_ulong).wrapping_div(elt_sz))
    {
        ck_assert_failed(
            b"ck.c\0" as *const u8 as *const libc::c_char,
            349 as libc::c_int as libc::c_uint,
            b"x <= UINT32_MAX && n <= UINT32_MAX - x && n+x <= SIZE_MAX/elt_sz\0"
                as *const u8 as *const libc::c_char,
        );
    }
    let mut ptr: *mut libc::c_void = realloc(
        *list,
        n.wrapping_add(x).wrapping_mul(elt_sz),
    );
    if ptr.is_null() {
        ck_assert_failed(
            b"ck.c\0" as *const u8 as *const libc::c_char,
